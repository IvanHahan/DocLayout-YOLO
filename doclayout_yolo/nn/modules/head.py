...
class v10DetectWithEL(Detect):

    max_det = -1

    def __init__(self, nc=80, ch=()):
        super().__init__(nc, ch)
        self.no = self.no + self.reg_max * 2
        c3 = max(ch[0], min(self.nc, 100))  # channels
        self.cv3 = nn.ModuleList(
            nn.Sequential(
                nn.Sequential(Conv(x, x, 3, g=x), Conv(x, c3, 1)),
        ...
        self.child = nn.ModuleList(
            nn.Sequential(
                Conv(x, c4, 3), Conv(c4, c4, 3), nn.Conv2d(c4, self.reg_max, 1)
            )
            for x in ch
        )
        self.parent = nn.ModuleList(
            nn.Sequential(
                Conv(x, c4, 3), Conv(c4, c4, 3), nn.Conv2d(c4, self.reg_max, 1)
            )
            for x in ch
        ...
        self.one2one_cv2 = copy.deepcopy(self.cv2)
        self.one2one_cv3 = copy.deepcopy(self.cv3)
        self.one2one_child = copy.deepcopy(self.child)
        self.one2one_parent = copy.deepcopy(self.parent)
    ...
    def forward_feat(self, x, cv2, cv3, cv_child, cv_parent):
        y = []
        for i in range(self.nl):
            y.append(
                torch.cat(
                    (cv2[i](x[i]), cv3[i](x[i]), cv_child[i](x[i]), cv_parent[i](x[i])),
                    1,
                )
            )
        return y
    ...
    def bias_init(self):
        ...
        # cf = torch.bincount(torch.tensor(np.concatenate(dataset.labels, 0)[:, 0]).long(), minlength=nc) + 1
        # ncf = math.log(0.6 / (m.nc - 0.999999)) if cf is None else torch.log(cf / cf.sum())  # nominal class frequency
        for a, b, c, p, s in zip(
            m.one2one_cv2, m.one2one_cv3, m.one2one_child, m.one2one_parent, m.stride
        ):  # from
            a[-1].bias.data[:] = 1.0  # box
            b[-1].bias.data[: m.nc] = math.log(
                5 / m.nc / (640 / s) ** 2
